    ...
    if isfield(c3d, 'ezc3d')
        ...
        for i = 1:length(names)
            if length(names{i}) > length('Angles') && ...
                ...
                out.(names{i}) = squeeze(c3d.ezc3d.data.points(:, i, :))';
            end
        end
        outInfo.frequency = c3d.ezc3d.parameters.POINT.RATE.DATA;
        outInfo.units.ALLANGLES = c3d.ezc3d.parameters.POINT.ANGLE_UNITS.DATA{1};
